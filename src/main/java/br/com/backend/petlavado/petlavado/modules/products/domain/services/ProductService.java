...
public class ProductService {
  ...
  public List<Product> getProductBySearchTerm(String searchTerm) {
    ...
    return productRepository.findProductByDescriptionContainingIgnoreCaseOrderByStore(
      searchTerm
    );
  }
  ...
  public List<Product> getProductsByLocation(String geolocation) {
    ...
    for (Product product : products) {
      double distance = formula.calculateDistance(
        geolocation,
        product.getStore().getGeolocation()
      );
  ...
  public Product createProduct(Integer storeId, ProductDto data) {
    ...
    return productRepository.save(
      new Product(data.description(), data.value(), data.imageUrl(), store)
    );
  }
  ...
  public Product updateProduct(
    ...
    if (!existingProduct.getStore().getId().equals(storeId)) {
      throw new ResponseStatusException(
        ...
        "Product with ID " +
        productId +
        " does not belong to store with ID " +
        storeId
      );
    }
  ...
  public void deleteProduct(Integer storeId, Integer productId) {
    ...
    Product product = getProductOrNull(productId)
      .orElseThrow(
        () ->
          new ResponseStatusException(
            HttpStatus.NOT_FOUND,
